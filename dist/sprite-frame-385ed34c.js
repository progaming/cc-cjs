...
var Atlas = function () {
  ...
  _proto.insertSpriteFrame = function insertSpriteFrame(spriteFrame) {
    ...
    if (info) {
      ...
      if (jsonAsset.legacyCC.internal.dynamicAtlasManager.textureBleeding) {
        if (width <= 8 || height <= 8) {
...
var DynamicAtlasTexture = function (_Texture2D) {
  jsonAsset._inheritsLoose(DynamicAtlasTexture, _Texture2D);
...
var DynamicAtlasManager = function () {
  ...
  jsonAsset._createClass(DynamicAtlasManager, [{
    key: "enabled",
    get: function get() {
    ...
    set: function set(value) {
      ...
      if (value) {
        this.reset();
        jsonAsset.legacyCC.director.on(jsonAsset.legacyCC.Director.EVENT_BEFORE_SCENE_LAUNCH, this.beforeSceneLoad, this);
      } else {
        this.reset();
        jsonAsset.legacyCC.director.off(jsonAsset.legacyCC.Director.EVENT_BEFORE_SCENE_LAUNCH, this.beforeSceneLoad, this);
      }
...
jsonAsset.legacyCC.internal.dynamicAtlasManager = dynamicAtlasManager;
...
var SpriteFrame = (_dec = jsonAsset.ccclass('cc.SpriteFrame'), _dec(_class = (_temp = function (_Asset) {
  jsonAsset._inheritsLoose(SpriteFrame, _Asset);

  SpriteFrame.createWithImage = function createWithImage(imageSourceOrImageAsset) {
    var img = imageSourceOrImageAsset instanceof jsonAsset.ImageAsset ? imageSourceOrImageAsset : new jsonAsset.ImageAsset(imageSourceOrImageAsset);
  ...
  jsonAsset._createClass(SpriteFrame, [{
    key: "insetTop",
    get: function get() {
  ...
  function SpriteFrame() {
    ...
    _this.unbiasUV = [];
    _this.uvSliced = [];
    _this._rect = new jsonAsset.Rect$1();
    _this._offset = new jsonAsset.Vec2();
    _this._originalSize = new jsonAsset.Size$1();
  ...
  _proto.checkRect = function checkRect(texture) {
    ...
    if (maxX > texture.width) {
      jsonAsset.errorID(3300, this.name + "/" + texture.name, maxX, texture.width);
      return false;
    }

    if (maxY > texture.height) {
      jsonAsset.errorID(3301, this.name + "/" + texture.name, maxY, texture.height);
      return false;
    }
  ...
  _proto._calculateUV = function _calculateUV() {
    ...
    this.uvHash = jsonAsset.murmurhash2_32_gc(uvHashStr, 666);
  ...
  _proto._checkPackable = function _checkPackable() {
    ...
    if (!(texture instanceof jsonAsset.Texture2D) || texture.isCompressed) {
  ...
  _proto._deserialize = function _deserialize(serializeData, handle) {
    ...
    if (rect) {
      this._rect = new jsonAsset.Rect$1(rect.x, rect.y, rect.width, rect.height);
    }

    var offset = data.offset;

    if (data.offset) {
      this._offset = new jsonAsset.Vec2(offset.x, offset.y);
    }

    var originalSize = data.originalSize;

    if (data.originalSize) {
      this._originalSize = new jsonAsset.Size$1(originalSize.width, originalSize.height);
    }
  ...
  _proto._refreshTexture = function _refreshTexture(texture) {
    ...
    if (this._rect.width === 0 || this._rect.height === 0 || !this.checkRect(tex)) {
      config.rect = new jsonAsset.Rect$1(0, 0, tex.width, tex.height);
      isReset = true;
    }

    if (this._originalSize.width === 0 || this._originalSize.height === 0 || isReset) {
      config.originalSize = new jsonAsset.Size$1(tex.width, tex.height);
      isReset = true;
    }
  ...
  return SpriteFrame;
}(jsonAsset.Asset), _temp)) || _class);
jsonAsset.legacyCC.SpriteFrame = SpriteFrame;
