...
var StdMorphRendering = function () {
  function StdMorphRendering(mesh, gfxDevice) {
    ...
    for (var iSubMesh = 0; iSubMesh < nSubMeshes; ++iSubMesh) {
      ...
      if ( subMeshMorph.targets.length > jsonAsset.UBOMorph.MAX_MORPH_TARGET_COUNT) {
  ...
  _proto.createInstance = function createInstance() {
    ...
    return {
      ...
      requiredPatches: function requiredPatches(subMeshIndex) {
        jsonAsset.assertIsNonNullable(_this._mesh.struct.morph);
        ...
        if (subMeshMorph.attributes.includes(jsonAsset.AttributeName.ATTR_POSITION)) {
          patches.push({
        ...
        if (subMeshMorph.attributes.includes(jsonAsset.AttributeName.ATTR_NORMAL)) {
          patches.push({
        ...
        if (subMeshMorph.attributes.includes(jsonAsset.AttributeName.ATTR_TANGENT)) {
          patches.push({
      ...
      destroy: function destroy() {
        for (var _iterator = jsonAsset._createForOfIteratorHelperLoose(subMeshInstances), _step; !(_step = _iterator()).done;) {
...
var GpuComputing = function () {
  ...
  _proto2.destroy = function destroy() {
    for (var _iterator2 = jsonAsset._createForOfIteratorHelperLoose(this._attributes), _step2; !(_step2 = _iterator2()).done;) {
  ...
  _proto2.createInstance = function createInstance() {
    ...
    return {
      ...
      adaptPipelineState: function adaptPipelineState(descriptorSet) {
        for (var _iterator3 = jsonAsset._createForOfIteratorHelperLoose(_this2._attributes), _step3; !(_step3 = _iterator3()).done;) {
          var attribute = _step3.value;
          var binding = void 0;

          switch (attribute.name) {
            case jsonAsset.AttributeName.ATTR_POSITION:
              binding = jsonAsset.UNIFORM_POSITION_MORPH_TEXTURE_BINDING;
              break;

            case jsonAsset.AttributeName.ATTR_NORMAL:
              binding = jsonAsset.UNIFORM_NORMAL_MORPH_TEXTURE_BINDING;
              break;

            case jsonAsset.AttributeName.ATTR_TANGENT:
              binding = jsonAsset.UNIFORM_TANGENT_MORPH_TEXTURE_BINDING;
              break;

            default:
              jsonAsset.warn('Unexpected attribute!');
              break;
          }
        ...
        descriptorSet.bindBuffer(jsonAsset.UBOMorph.BINDING, morphUniforms.buffer);
...
var CpuComputing = function () {
  ...
  jsonAsset._createClass(CpuComputing, [{
    key: "data",
    get: function get() {
...
var CpuComputingRenderingInstance = function () {
  ...
  _proto4.setWeights = function setWeights(weights) {
    for (var iAttribute = 0; iAttribute < this._attributes.length; ++iAttribute) {
      ...
      jsonAsset.assertIsTrue(weights.length === attributeMorph.targets.length);
  ...
  _proto4.adaptPipelineState = function adaptPipelineState(descriptorSet) {
    for (var _iterator4 = jsonAsset._createForOfIteratorHelperLoose(this._attributes), _step4; !(_step4 = _iterator4()).done;) {
    ...
    descriptorSet.bindBuffer(jsonAsset.UBOMorph.BINDING, this._morphUniforms.buffer);
    descriptorSet.update();
  };
...
var MorphUniforms = function () {
  function MorphUniforms(gfxDevice, targetCount) {
    ...
    this._localBuffer = new DataView(new ArrayBuffer(jsonAsset.UBOMorph.SIZE));
    this._remoteBuffer = gfxDevice.createBuffer(new jsonAsset.BufferInfo(jsonAsset.BufferUsageBit.UNIFORM | jsonAsset.BufferUsageBit.TRANSFER_DST, jsonAsset.MemoryUsageBit.HOST | jsonAsset.MemoryUsageBit.DEVICE, jsonAsset.UBOMorph.SIZE, jsonAsset.UBOMorph.SIZE));
  }
  ...
  _proto5.setWeights = function setWeights(weights) {
    jsonAsset.assertIsTrue(weights.length === this._targetCount);

    for (var iWeight = 0; iWeight < weights.length; ++iWeight) {
      this._localBuffer.setFloat32(jsonAsset.UBOMorph.OFFSET_OF_WEIGHTS + 4 * iWeight, weights[iWeight], jsonAsset.legacyCC.sys.isLittleEndian);
    }
  };

  _proto5.setMorphTextureInfo = function setMorphTextureInfo(width, height) {
    this._localBuffer.setFloat32(jsonAsset.UBOMorph.OFFSET_OF_DISPLACEMENT_TEXTURE_WIDTH, width, jsonAsset.legacyCC.sys.isLittleEndian);

    this._localBuffer.setFloat32(jsonAsset.UBOMorph.OFFSET_OF_DISPLACEMENT_TEXTURE_HEIGHT, height, jsonAsset.legacyCC.sys.isLittleEndian);
  };

  _proto5.setVerticesCount = function setVerticesCount(count) {
    this._localBuffer.setFloat32(jsonAsset.UBOMorph.OFFSET_OF_VERTICES_COUNT, count, jsonAsset.legacyCC.sys.isLittleEndian);
  };
  ...
  jsonAsset._createClass(MorphUniforms, [{
    key: "buffer",
    get: function get() {
...
function createVec4TextureFactory(gfxDevice, vec4Capacity) {
  var hasFeatureFloatTexture = gfxDevice.hasFeature(jsonAsset.Feature.TEXTURE_FLOAT);
  var pixelRequired;
  var pixelFormat;
  var pixelBytes;
  ...
  if (hasFeatureFloatTexture) {
    ...
    pixelBytes = 16;
    pixelFormat = jsonAsset.Texture2D.PixelFormat.RGBA32F;
    UpdateViewConstructor = Float32Array;
  } else {
    pixelRequired = 4 * vec4Capacity;
    pixelBytes = 4;
    pixelFormat = jsonAsset.Texture2D.PixelFormat.RGBA8888;
  ...
  return {
    ...
    create: function create() {
      ...
      var textureAsset = new jsonAsset.Texture2D();
      textureAsset.setFilters(jsonAsset.Texture2D.Filter.NEAREST, jsonAsset.Texture2D.Filter.NEAREST);
      textureAsset.setMipFilter(jsonAsset.Texture2D.Filter.NONE);
      textureAsset.setWrapMode(jsonAsset.Texture2D.WrapMode.CLAMP_TO_EDGE, jsonAsset.Texture2D.WrapMode.CLAMP_TO_EDGE, jsonAsset.Texture2D.WrapMode.CLAMP_TO_EDGE);
      textureAsset.image = image;

      if (!textureAsset.getGFXTexture()) {
        jsonAsset.warn('Unexpected: failed to create morph texture?');
      }

      var sampler = jsonAsset.samplerLib.getSampler(gfxDevice, textureAsset.getSamplerHash());
      return {
        get texture() {
...
function bestSizeToHavePixels(nPixels) {
  ...
  var aligned = jsonAsset.nextPow2$1(nPixels);
  var epxSum = jsonAsset.log2(aligned);
  var h = epxSum >> 1;
...
var Mesh = (_dec = jsonAsset.ccclass('cc.Mesh'), _dec(_class = (_class2 = (_temp = function (_Asset) {
  jsonAsset._inheritsLoose(Mesh, _Asset);

  jsonAsset._createClass(Mesh, [{
    key: "_nativeAsset",
    get: function get() {
    ...
    get: function get() {
      if (!this._hash) {
        this._hash = jsonAsset.murmurhash2_32_gc(this._data, 666);
      }
  ...
  function Mesh() {
    ...
    jsonAsset._initializerDefineProperty(_this, "_struct", _descriptor, jsonAsset._assertThisInitialized(_this));

    jsonAsset._initializerDefineProperty(_this, "_hash", _descriptor2, jsonAsset._assertThisInitialized(_this));
  ...
  _proto.initialize = function initialize() {
    ...
    for (var i = 0; i < this._struct.primitives.length; i++) {
      var prim = this._struct.primitives[i];
      ...
      if (prim.indexView) {
        ...
        if (dstStride === 4 && !gfxDevice.hasFeature(jsonAsset.Feature.ELEMENT_INDEX_UINT)) {
          var vertexCount = this._struct.vertexBundles[prim.vertexBundelIndices[0]].view.count;

          if (vertexCount >= 65536) {
            jsonAsset.warnID(10001, vertexCount, 65536);
            continue;
          } else {
            dstStride >>= 1;
            dstSize >>= 1;
        ...
        indexBuffer = gfxDevice.createBuffer(new jsonAsset.BufferInfo(jsonAsset.BufferUsageBit.INDEX, jsonAsset.MemoryUsageBit.DEVICE, dstSize, dstStride));
      ...
      if (prim.vertexBundelIndices.length > 0) {
        ...
        var _vertexBundle = this._struct.vertexBundles[idx];
        var attrs = _vertexBundle.attributes;

        for (var j = 0; j < attrs.length; ++j) {
          var attr = attrs[j];
          gfxAttributes[j] = new jsonAsset.Attribute(attr.name, attr.format, attr.isInstanced, attr.stream, attr.isInstanced, attr.location);
        }
      }

      var subMesh = new jsonAsset.RenderingSubMesh(vbReference, gfxAttributes, prim.primitiveMode, indexBuffer);
      subMesh.mesh = this;
  ...
  _proto.getBoneSpaceBounds = function getBoneSpaceBounds(skeleton) {
    ...
    for (var i = 0; i < bindposes.length; i++) {
      bounds.push(new jsonAsset.AABB(Infinity, Infinity, Infinity, -Infinity, -Infinity, -Infinity));
      valid.push(false);
    }
    ...
    for (var p = 0; p < primitives.length; p++) {
      var joints = this.readAttribute(p, jsonAsset.AttributeName.ATTR_JOINTS);
      var weights = this.readAttribute(p, jsonAsset.AttributeName.ATTR_WEIGHTS);
      var positions = this.readAttribute(p, jsonAsset.AttributeName.ATTR_POSITION);
      ...
      for (var _i = 0; _i < vertCount; _i++) {
        jsonAsset.Vec3.set(v3_1, positions[3 * _i + 0], positions[3 * _i + 1], positions[3 * _i + 2]);
        ...
        for (var j = 0; j < 4; ++j) {
          ...
          jsonAsset.Vec3.transformMat4(v3_2, v3_1, bindposes[joint]);
          valid[joint] = true;
          var b = bounds[joint];
          jsonAsset.Vec3.min(b.center, b.center, v3_2);
          jsonAsset.Vec3.max(b.halfExtents, b.halfExtents, v3_2);
        }
      }
    }
    ...
    for (var _i2 = 0; _i2 < bindposes.length; _i2++) {
      ...
      if (!valid[_i2]) {
        ...
        jsonAsset.AABB.fromPoints(_b, _b.center, _b.halfExtents);
      }
    }
  ...
  _proto.merge = function merge(mesh, worldMatrix, validate) {
    ...
    var rotate = worldMatrix && new jsonAsset.Quat();
    var boundingBox = worldMatrix && new jsonAsset.AABB();
    ...
    if (!this._initialized) {
      ...
      if (worldMatrix) {
        if (struct.maxPosition && struct.minPosition) {
          jsonAsset.Vec3.add(boundingBox.center, struct.maxPosition, struct.minPosition);
          jsonAsset.Vec3.multiplyScalar(boundingBox.center, boundingBox.center, 0.5);
          jsonAsset.Vec3.subtract(boundingBox.halfExtents, struct.maxPosition, struct.minPosition);
          jsonAsset.Vec3.multiplyScalar(boundingBox.halfExtents, boundingBox.halfExtents, 0.5);
          jsonAsset.AABB.transform(boundingBox, boundingBox, worldMatrix);
          jsonAsset.Vec3.add(struct.maxPosition, boundingBox.center, boundingBox.halfExtents);
          jsonAsset.Vec3.subtract(struct.minPosition, boundingBox.center, boundingBox.halfExtents);
        }
        ...
        for (var i = 0; i < struct.vertexBundles.length; i++) {
          ...
          for (var j = 0; j < vtxBdl.attributes.length; j++) {
            if (vtxBdl.attributes[j].name === jsonAsset.AttributeName.ATTR_POSITION || vtxBdl.attributes[j].name === jsonAsset.AttributeName.ATTR_NORMAL) {
              ...
              for (var vtxIdx = 0; vtxIdx < vertexCount; vtxIdx++) {
                ...
                switch (vtxBdl.attributes[j].name) {
                  ...
                  case jsonAsset.AttributeName.ATTR_NORMAL:
                    jsonAsset.Vec3.transformQuat(vec3_temp, vec3_temp, rotate);
                    break;
                }
    ...
    for (var _i3 = 0; _i3 < this._struct.vertexBundles.length; ++_i3) {
      ...
      for (var _iterator = jsonAsset._createForOfIteratorHelperLoose(bundle.attributes), _step; !(_step = _iterator()).done;) {
        var attr = _step.value;
        dstVBOffset = 0;
        hasAttr = false;

        for (var _iterator2 = jsonAsset._createForOfIteratorHelperLoose(dstBundle.attributes), _step2; !(_step2 = _iterator2()).done;) {
          ...
          dstVBOffset += jsonAsset.FormatInfos[dstAttr.format].size;
        }

        if (hasAttr) {
          attrSize = jsonAsset.FormatInfos[attr.format].size;
          ...
          for (var v = 0; v < dstBundle.view.count; ++v) {
            ...
            if ((attr.name === jsonAsset.AttributeName.ATTR_POSITION || attr.name === jsonAsset.AttributeName.ATTR_NORMAL) && worldMatrix) {
        ...
        srcAttrOffset += jsonAsset.FormatInfos[attr.format].size;
      }
    ...
    for (var _i4 = 0; _i4 < this._struct.primitives.length; ++_i4) {
      ...
      for (var _iterator3 = jsonAsset._createForOfIteratorHelperLoose(prim.vertexBundelIndices), _step3; !(_step3 = _iterator3()).done;) {
    ...
    if (meshStruct.minPosition && mesh._struct.minPosition && meshStruct.maxPosition && mesh._struct.maxPosition) {
      if (worldMatrix) {
        jsonAsset.Vec3.add(boundingBox.center, mesh._struct.maxPosition, mesh._struct.minPosition);
        jsonAsset.Vec3.multiplyScalar(boundingBox.center, boundingBox.center, 0.5);
        jsonAsset.Vec3.subtract(boundingBox.halfExtents, mesh._struct.maxPosition, mesh._struct.minPosition);
        jsonAsset.Vec3.multiplyScalar(boundingBox.halfExtents, boundingBox.halfExtents, 0.5);
        jsonAsset.AABB.transform(boundingBox, boundingBox, worldMatrix);
        jsonAsset.Vec3.add(vec3_temp, boundingBox.center, boundingBox.halfExtents);
        jsonAsset.Vec3.max(meshStruct.maxPosition, meshStruct.maxPosition, vec3_temp);
        jsonAsset.Vec3.subtract(vec3_temp, boundingBox.center, boundingBox.halfExtents);
        jsonAsset.Vec3.min(meshStruct.minPosition, meshStruct.minPosition, vec3_temp);
      } else {
        jsonAsset.Vec3.min(meshStruct.minPosition, meshStruct.minPosition, mesh._struct.minPosition);
        jsonAsset.Vec3.max(meshStruct.maxPosition, meshStruct.maxPosition, mesh._struct.maxPosition);
      }
    }
  ...
  _proto.readAttribute = function readAttribute(primitiveIndex, attributeName) {
    ...
    this._accessAttribute(primitiveIndex, attributeName, function (vertexBundle, iAttribute) {
      ...
      var StorageConstructor = jsonAsset.getTypedArrayConstructor(jsonAsset.FormatInfos[format]);
      ...
      var formatInfo = jsonAsset.FormatInfos[format];
      var reader = getReader(inputView, format);
  ...
  _proto.copyIndices = function copyIndices(primitiveIndex, outputArray) {
    ...
    var indexFormat = primitive.indexView.stride === 1 ? jsonAsset.Format.R8UI : primitive.indexView.stride === 2 ? jsonAsset.Format.R16UI : jsonAsset.Format.R32UI;
    var reader = getReader(new DataView(this._data.buffer), indexFormat);

    for (var i = 0; i < indexCount; ++i) {
      outputArray[i] = reader(primitive.indexView.offset + jsonAsset.FormatInfos[indexFormat].size * i);
    }
  ...
  _proto._accessAttribute = function _accessAttribute(primitiveIndex, attributeName, accessor) {
    ...
    for (var _iterator4 = jsonAsset._createForOfIteratorHelperLoose(primitive.vertexBundelIndices), _step4; !(_step4 = _iterator4()).done;) {
  ...
  _proto._createVertexBuffers = function _createVertexBuffers(gfxDevice, data) {
    return this._struct.vertexBundles.map(function (vertexBundle) {
      var vertexBuffer = gfxDevice.createBuffer(new jsonAsset.BufferInfo(jsonAsset.BufferUsageBit.VERTEX, jsonAsset.MemoryUsageBit.DEVICE, vertexBundle.view.length, vertexBundle.view.stride));
  ...
  return Mesh;
}(jsonAsset.Asset), _temp), (_descriptor = jsonAsset._applyDecoratedDescriptor(_class2.prototype, "_struct", [jsonAsset.serializable], {
  configurable: true,
  enumerable: true,
  writable: true,
...
}), _descriptor2 = jsonAsset._applyDecoratedDescriptor(_class2.prototype, "_hash", [jsonAsset.serializable], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function initializer() {
...
function getOffset(attributes, attributeIndex) {
  ...
  for (var i = 0; i < attributeIndex; ++i) {
    ...
    result += jsonAsset.FormatInfos[attribute.format].size;
  }

  return result;
}

var isLittleEndian = jsonAsset.sys.isLittleEndian;

function getComponentByteLength(format) {
  var info = jsonAsset.FormatInfos[format];
...
function getReader(dataView, format) {
  ...
  switch (info.type) {
    case jsonAsset.FormatType.UNORM:
      {
        switch (stride) {
          case 1:
    ...
    case jsonAsset.FormatType.SNORM:
      {
        switch (stride) {
          case 1:
            return function (offset) {
    ...
    case jsonAsset.FormatType.UINT:
      {
        switch (stride) {
          case 1:
            return function (offset) {
    ...
    case jsonAsset.FormatType.FLOAT:
      {
        return function (offset) {
