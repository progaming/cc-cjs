...
var Action = function () {
  ...
  _proto.step = function step(dt) {
    jsonAsset.logID(1006);
  };

  _proto.update = function update(dt) {
    jsonAsset.logID(1007);
  };
...
var FiniteTimeAction = function (_Action) {
  jsonAsset._inheritsLoose(FiniteTimeAction, _Action);
...
var Speed = function (_Action2) {
  jsonAsset._inheritsLoose(Speed, _Action2);
...
var ActionManager = function () {
  ...
  _proto.removeActionByTag = function removeActionByTag(tag, target) {
    ...
    if (tag === Action.TAG_INVALID) jsonAsset.logID(1002);
  ...
  _proto.getActionByTag = function getActionByTag(tag, target) {
    if (tag === Action.TAG_INVALID) jsonAsset.logID(1004);
  ...
  _proto.purgeSharedManager = function purgeSharedManager() {
    jsonAsset.legacyCC.director.getScheduler().unscheduleUpdate(this);
  };
  ...
  _proto.update = function update(dt) {
    ...
    for (var elt = 0; elt < locTargets.length; elt++) {
      ...
      if (target instanceof jsonAsset.CCObject && !target.isValid) {
...
var TweenSystem = function (_System) {
  jsonAsset._inheritsLoose(TweenSystem, _System);
  ...
  jsonAsset._createClass(TweenSystem, [{
    key: "ActionManager",
    get: function get() {
...
index.director.on(index.Director.EVENT_INIT, function () {
  ...
  index.director.registerSystem(TweenSystem.ID, sys, index.System.Priority.MEDIUM);
});

var ActionInstant = function (_FiniteTimeAction) {
  jsonAsset._inheritsLoose(ActionInstant, _FiniteTimeAction);
...
var Show = function (_ActionInstant) {
  jsonAsset._inheritsLoose(Show, _ActionInstant);
  ...
  _proto2.update = function update(dt) {
    var _renderComps = this.target.getComponentsInChildren(renderableComponent.RenderableComponent);
...
var Hide = function (_ActionInstant2) {
  jsonAsset._inheritsLoose(Hide, _ActionInstant2);
...
var ToggleVisibility = function (_ActionInstant3) {
  jsonAsset._inheritsLoose(ToggleVisibility, _ActionInstant3);
...
var RemoveSelf = function (_ActionInstant4) {
  jsonAsset._inheritsLoose(RemoveSelf, _ActionInstant4);
...
var CallFunc = function (_ActionInstant5) {
  jsonAsset._inheritsLoose(CallFunc, _ActionInstant5);
...
var ActionInterval = function (_FiniteTimeAction) {
  jsonAsset._inheritsLoose(ActionInterval, _FiniteTimeAction);
  ...
  _proto.initWithDuration = function initWithDuration(d) {
    this._duration = d === 0 ? jsonAsset.macro.FLT_EPSILON : d;
    this._elapsed = 0;
...
var Sequence = function (_ActionInterval) {
  jsonAsset._inheritsLoose(Sequence, _ActionInterval);
  ...
  function Sequence(tempArray) {
    ...
    if (paramArray.length === 1) {
      jsonAsset.errorID(1019);
      return jsonAsset._assertThisInitialized(_this2);
    }

    var last = paramArray.length - 1;
    if (last >= 0 && paramArray[last] == null) jsonAsset.logID(1015);
...
var Repeat = function (_ActionInterval2) {
  jsonAsset._inheritsLoose(Repeat, _ActionInterval2);
...
var RepeatForever = function (_ActionInterval3) {
  jsonAsset._inheritsLoose(RepeatForever, _ActionInterval3);
...
var Spawn = function (_ActionInterval4) {
  jsonAsset._inheritsLoose(Spawn, _ActionInterval4);
  ...
  function Spawn(tempArray) {
    ...
    if (paramArray.length === 1) {
      jsonAsset.errorID(1020);
      return jsonAsset._assertThisInitialized(_this5);
    }
...
function spawn(tempArray) {
  ...
  if (paramArray.length > 0 && paramArray[paramArray.length - 1] == null) jsonAsset.logID(1015);
  var prev = paramArray[0];
...
var DelayTime = function (_ActionInterval5) {
  jsonAsset._inheritsLoose(DelayTime, _ActionInterval5);
...
var ReverseTime = function (_ActionInterval6) {
  jsonAsset._inheritsLoose(ReverseTime, _ActionInterval6);
  ...
  _proto7.initWithAction = function initWithAction(action) {
    if (!action) {
      jsonAsset.errorID(1028);
      return false;
    }

    if (action === this._other) {
      jsonAsset.errorID(1029);
      return false;
    }
...
function TweenOptionChecker(opts) {
  var header = ' [Tween:] ';
  var message = " option is not support in v + " + jsonAsset.engineVersion;
  var _opts = opts;

  if (_opts.delay) {
    jsonAsset.warn(header + "delay" + message);
  }

  if (_opts.repeat) {
    jsonAsset.warn(header + "repeat" + message);
  }

  if (_opts.repeatDelay) {
    jsonAsset.warn(header + "repeatDelay" + message);
  }

  if (_opts.interpolation) {
    jsonAsset.warn(header + "interpolation" + message);
  }

  if (_opts.onStop) {
    jsonAsset.warn(header + "onStop" + message);
  }
}

var TweenAction = function (_ActionInterval) {
  jsonAsset._inheritsLoose(TweenAction, _ActionInterval);
  ...
  function TweenAction(duration, props, opts) {
    ...
    if (opts == null) {
      ...
      if (opts.easing && typeof opts.easing === 'string') {
        ...
        opts.easing = jsonAsset.easing[easingName];

        if (!opts.easing) {
          jsonAsset.warnID(1031, easingName);
        }
      }
    }
    ...
    for (var name in props) {
      ...
      if (value.value !== undefined && (value.easing || value.progress)) {
        if (typeof value.easing === 'string') {
          customEasing = jsonAsset.easing[value.easing];
          if (!customEasing) jsonAsset.warnID(1031, value.easing);
        } else {
...
var SetAction = function (_ActionInstant) {
  jsonAsset._inheritsLoose(SetAction, _ActionInstant);
...
var Tween = function () {
  ...
  _proto.start = function start() {
    if (!this._target) {
      jsonAsset.warn('Please set target to tween first');
      return this;
    }
...
Tween._tmp_args = [];
jsonAsset.legacyCC.Tween = Tween;
function tween(target) {
  return new Tween(target);
}
jsonAsset.legacyCC.tween = tween;
function tweenUtil(target) {
  jsonAsset.warn('tweenUtil\' is deprecated, please use \'tween\' instead ');
  return new Tween(target);
}
jsonAsset.legacyCC.tweenUtil = tweenUtil;
