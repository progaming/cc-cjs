...
var TextureBufferPool = function () {
  function TextureBufferPool(device) {
    this._device = void 0;
    this._format = jsonAsset.Format.UNKNOWN;
    this._formatSize = 0;
    this._chunks = [];
    this._chunkCount = 0;
    this._handles = [];
    this._region0 = new jsonAsset.BufferTextureCopy();
    this._region1 = new jsonAsset.BufferTextureCopy();
    this._region2 = new jsonAsset.BufferTextureCopy();
  ...
  _proto.initialize = function initialize(info) {
    var formatInfo = jsonAsset.FormatInfos[info.format];
    this._format = info.format;
    this._formatSize = formatInfo.size;
    this._channels = formatInfo.count;
    this._bufferViewCtor = jsonAsset.getTypedArrayConstructor(formatInfo);
  ...
  _proto.createChunk = function createChunk(length) {
    ...
    jsonAsset.debug$1("TextureBufferPool: Allocate chunk " + this._chunkCount + ", size: " + texSize + ", format: " + this._format);

    var texture = this._device.createTexture(new jsonAsset.TextureInfo(jsonAsset.TextureType.TEX2D, jsonAsset.TextureUsageBit.SAMPLED | jsonAsset.TextureUsageBit.TRANSFER_DST, this._format, length, length, jsonAsset.TextureFlagBit.IMMUTABLE));
