...
var vec3_temp = new jsonAsset.Vec3();
...
function fillVertices3D(node, renderer, renderData, color) {
  ...
  for (var i = 0; i < vertexCount; i++) {
    var vert = dataList[i];
    jsonAsset.Vec3.set(vec3_temp, vert.x, vert.y, 0);
    jsonAsset.Vec3.transformMat4(vec3_temp, vec3_temp, _worldMatrix);
    vBuf[vertexOffset++] = vec3_temp.x;
    vBuf[vertexOffset++] = vec3_temp.y;
    vBuf[vertexOffset++] = vec3_temp.z;
    vBuf[vertexOffset++] = vert.u;
    vBuf[vertexOffset++] = vert.v;
    jsonAsset.Color$1.toArray(vBuf, color, vertexOffset);
    vertexOffset += 4;
  }
...
var _curColor = new jsonAsset.Color$1();
...
var graphicsAssembler = {
  ...
  stroke: function stroke(graphics) {
    jsonAsset.Color$1.copy(_curColor, graphics.strokeColor);
  ...
  fill: function fill(graphics) {
    jsonAsset.Color$1.copy(_curColor, graphics.fillColor);
  ...
  _expandStroke: function _expandStroke(graphics$1) {
    var w = graphics$1.lineWidth * 0.5;
    var lineCap = graphics$1.lineCap;
    var lineJoin = graphics$1.lineJoin;
    var miterLimit = graphics$1.miterLimit;
    _impl = graphics$1.impl;
    ...
    for (var _i = _impl.pathOffset, l = _impl.pathLength; _i < l; _i++) {
      ...
      if (lineJoin === graphics.LineJoin.ROUND) {
      ...
      if (!path.closed) {
        if (lineCap === graphics.LineCap.ROUND) {
    ...
    var meshBuffer = _renderData = this.getRenderData(graphics$1, vertexCount);
    ...
    for (var _i2 = _impl.pathOffset, _l = _impl.pathLength; _i2 < _l; _i2++) {
      ...
      if (!loop) {
        var dPos = new graphics.Point(p1.x, p1.y);
        dPos.subtract(p0);
        dPos.normalize();
        var dx = dPos.x;
        var dy = dPos.y;

        if (lineCap === graphics.LineCap.BUTT) {
          this._buttCapStart(p0, dx, dy, w, 0);
        } else if (lineCap === graphics.LineCap.SQUARE) {
          this._buttCapStart(p0, dx, dy, w, w);
        } else if (lineCap === graphics.LineCap.ROUND) {
      ...
      for (var j = start; j < end; ++j) {
        ...
        } else if ((p1.flags & (graphics.PointFlags.PT_BEVEL | graphics.PointFlags.PT_INNERBEVEL)) !== 0) {
      ...
      if (loop) {
        ...
        var _dPos = new graphics.Point(p1.x, p1.y);
  ...
  _calculateJoins: function _calculateJoins(impl, w, lineJoin, miterLimit) {
    ...
    for (var _i5 = impl.pathOffset, l = impl.pathLength; _i5 < l; _i5++) {
      ...
      for (var j = 0; j < ptsLength; j++) {
        ...
        if (cross > 0) {
          p1.flags |= graphics.PointFlags.PT_LEFT;
        }

        limit = max(11, min(p0.len, p1.len) * iw);

        if (dmr2 * limit * limit < 1) {
          p1.flags |= graphics.PointFlags.PT_INNERBEVEL;
        }

        if (p1.flags & graphics.PointFlags.PT_CORNER) {
          if (dmr2 * miterLimit * miterLimit < 1 || lineJoin === graphics.LineJoin.BEVEL || lineJoin === graphics.LineJoin.ROUND) {
            p1.flags |= graphics.PointFlags.PT_BEVEL;
          }
        }

        if ((p1.flags & (graphics.PointFlags.PT_BEVEL | graphics.PointFlags.PT_INNERBEVEL)) !== 0) {
          path.bevel++;
        }
  ...
  _roundJoin: function _roundJoin(p0, p1, lw, rw, nCap) {
    ...
    if ((p1.flags & graphics.PointFlags.PT_LEFT) !== 0) {
      var out = this._chooseBevel(p1.flags & graphics.PointFlags.PT_INNERBEVEL, p0, p1, lw);
      ...
      var _out = this._chooseBevel(p1.flags & graphics.PointFlags.PT_INNERBEVEL, p0, p1, -rw);
  ...
  _bevelJoin: function _bevelJoin(p0, p1, lw, rw) {
    ...
    if (p1.flags & graphics.PointFlags.PT_LEFT) {
      var out = this._chooseBevel(p1.flags & graphics.PointFlags.PT_INNERBEVEL, p0, p1, lw);
      ...
      var _out2 = this._chooseBevel(p1.flags & graphics.PointFlags.PT_INNERBEVEL, p0, p1, -rw);
  ...
  _vSet: function _vSet(x, y, distance) {
    ...
    vData[dataOffset++] = y;
    vData[dataOffset++] = 0;
    jsonAsset.Color$1.toArray(vData, _curColor, dataOffset);
    dataOffset += 4;
...
graphics.Graphics.Assembler = graphicsAssemblerManager;
...
var _tmpRect = new jsonAsset.Rect$1();
...
var _labelDimensions = new jsonAsset.Size$1();

var _contentSize = new jsonAsset.Size$1();

var letterPosition = new jsonAsset.Vec2();
var _lineSpacing = 0;
var _fntConfig = null;
...
var bmfontUtils = {
  ...
  _updateFontFamily: function _updateFontFamily(comp) {
    ...
    sprite.shareLabelInfo.fontAtlas = fontAsset.fontDefDictionary;
    spriteFrame.dynamicAtlasManager.packToDynamicAtlas(comp, _spriteFrame);
  },
  _updateLabelInfo: function _updateLabelInfo(comp) {
    sprite.shareLabelInfo.hash = '';
    sprite.shareLabelInfo.margin = 0;
  },
  _updateProperties: function _updateProperties(comp) {
    ...
    if (_overflow === sprite.Overflow.NONE) {
      _isWrapText = false;
      _contentSize.width += sprite.shareLabelInfo.margin * 2;
      _contentSize.height += sprite.shareLabelInfo.margin * 2;
    } else if (_overflow === sprite.Overflow.RESIZE_HEIGHT) {
      _isWrapText = true;
      _contentSize.height += sprite.shareLabelInfo.margin * 2;
    } else {
    ...
    sprite.shareLabelInfo.lineHeight = _lineHeight;
    sprite.shareLabelInfo.fontSize = _fontSize;
  ...
  _multilineTextWrap: function _multilineTextWrap(nextTokenFunc) {
    ...
    for (var index = 0; index < textLen;) {
      ...
      for (var tmp = 0; tmp < tokenLen; ++tmp) {
        ...
        letterDef = sprite.shareLabelInfo.fontAtlas.getLetterDefinitionForChar(character, sprite.shareLabelInfo);
        ...
        var letterX = nextLetterX + letterDef.offsetX * _bmfontScale - sprite.shareLabelInfo.margin;

        if (_isWrapText && _maxLineWidth > 0 && nextTokenX > 0 && letterX + letterDef.w * _bmfontScale > _maxLineWidth && !sprite.isUnicodeSpace(character)) {
          _linesWidth.push(letterRight);
    ...
    if (_labelWidth <= 0) {
      _contentSize.width = parseFloat(longestLine.toFixed(2)) + sprite.shareLabelInfo.margin * 2;
    }

    if (_labelHeight <= 0) {
      _contentSize.height = parseFloat(_textDesiredHeight.toFixed(2)) + sprite.shareLabelInfo.margin * 2;
    }
  ...
  _getFontScale: function _getFontScale() {
    return _overflow === sprite.Overflow.SHRINK ? _bmfontScale : 1;
  },
  _getFirstWordLen: function _getFirstWordLen(text, startIndex, textLen) {
    var character = text.charAt(startIndex);

    if (sprite.isUnicodeCJK(character) || character === '\n' || sprite.isUnicodeSpace(character)) {
      return 1;
    }

    var len = 1;
    var letterDef = sprite.shareLabelInfo.fontAtlas.getLetterDefinitionForChar(character, sprite.shareLabelInfo);
    ...
    for (var index = startIndex + 1; index < textLen; ++index) {
      ...
      if (letterX + letterDef.w * _bmfontScale > _maxLineWidth && !sprite.isUnicodeSpace(character) && _maxLineWidth > 0) {
        return len;
      }

      nextLetterX += letterDef.xAdvance * _bmfontScale + _spacingX;

      if (character === '\n' || sprite.isUnicodeSpace(character) || sprite.isUnicodeCJK(character)) {
        break;
      }
  ...
  _recordPlaceholderInfo: function _recordPlaceholderInfo(letterIndex, _char) {
    ...
    _lettersInfo[letterIndex].hash = _char.charCodeAt(0) + sprite.shareLabelInfo.hash;
  ...
  _recordLetterInfo: function _recordLetterInfo(letterPosition, character, letterIndex, lineIndex) {
    ...
    var key = _char2 + sprite.shareLabelInfo.hash;
    _lettersInfo[letterIndex].line = lineIndex;
    _lettersInfo[letterIndex]["char"] = character;
    _lettersInfo[letterIndex].hash = key;
    _lettersInfo[letterIndex].valid = sprite.shareLabelInfo.fontAtlas.getLetter(key).valid;
  ...
  _alignText: function _alignText() {
    ...
    if (_overflow === sprite.Overflow.SHRINK) {
  ...
  _isHorizontalClamp: function _isHorizontalClamp() {
    ...
    for (var ctr = 0, l = _string.length; ctr < l; ++ctr) {
      ...
      if (letterInfo.valid) {
        var letterDef = sprite.shareLabelInfo.fontAtlas.getLetterDefinitionForChar(letterInfo["char"], sprite.shareLabelInfo);
  ...
  _updateQuads: function _updateQuads() {
    ...
    var texture = _spriteFrame ? _spriteFrame.texture : sprite.shareLabelInfo.fontAtlas.getTexture();
    ...
    for (var ctr = 0, l = _string.length; ctr < l; ++ctr) {
      ...
      var letterDef = sprite.shareLabelInfo.fontAtlas.getLetter(letterInfo.hash);
      ...
      if (_labelHeight > 0) {
        ...
        if (py - letterDef.h * _bmfontScale < _tailoredBottomY && _overflow === sprite.Overflow.CLAMP) {
      ...
      if (_labelWidth > 0) {
        if (this._isHorizontalClamped(px, lineIndex)) {
          if (_overflow === sprite.Overflow.CLAMP) {
            _tmpRect.width = 0;
          } else if (_overflow === sprite.Overflow.SHRINK) {
  ...
  _setupBMFontOverflowMetrics: function _setupBMFontOverflowMetrics() {
    ...
    if (_overflow === sprite.Overflow.RESIZE_HEIGHT) {
      newHeight = 0;
    }
...
var tempColor = new jsonAsset.Color$1(255, 255, 255, 255);
var bmfont = {
...
jsonAsset.addon(bmfont, bmfontUtils);

var _atlasWidth = 1024;
var _atlasHeight = 1024;
var _shareAtlas = null;
var letterFont = jsonAsset.mixin(bmfontUtils, {
  getAssemblerData: function getAssemblerData() {
    if (!_shareAtlas) {
      _shareAtlas = new sprite.LetterAtlas(_atlasWidth, _atlasHeight);
    }

    return _shareAtlas.getTexture();
  },
  _updateFontFamily: function _updateFontFamily(comp) {
    sprite.shareLabelInfo.fontAtlas = _shareAtlas;
    sprite.shareLabelInfo.fontFamily = this._getFontFamily(comp);
    var outline = comp.getComponent(deprecated.LabelOutline);

    if (outline && outline.enabled) {
      sprite.shareLabelInfo.isOutlined = true;
      sprite.shareLabelInfo.margin = outline.width;
      sprite.shareLabelInfo.out = outline.color.clone();
      sprite.shareLabelInfo.out.a = outline.color.a * comp.color.a / 255.0;
    } else {
      sprite.shareLabelInfo.isOutlined = false;
      sprite.shareLabelInfo.margin = 0;
    }
  },
  ...
  _updateLabelInfo: function _updateLabelInfo(comp) {
    sprite.shareLabelInfo.fontDesc = this._getFontDesc();
    sprite.shareLabelInfo.color = comp.color;
    sprite.shareLabelInfo.hash = sprite.computeHash(sprite.shareLabelInfo);
  },
  _getFontDesc: function _getFontDesc() {
    var fontDesc = sprite.shareLabelInfo.fontSize.toString() + "px ";
    fontDesc += sprite.shareLabelInfo.fontFamily;
...
var WHITE = new jsonAsset.Color$1(255, 255, 255, 255);
var letter = {
  createData: function createData(comp) {
    return comp.requestRenderData();
  },
...
jsonAsset.addon(letter, letterFont);

var Overflow = sprite.Label.Overflow;
var MAX_SIZE = 2048;

var _BASELINE_OFFSET = sprite.getBaselineOffset();
...
var _canvasSize = new jsonAsset.Size$1();
...
var _outlineColor = jsonAsset.Color$1.BLACK.clone();

var _shadowComp = null;

var _shadowColor = jsonAsset.Color$1.BLACK.clone();

var _canvasPadding = new jsonAsset.Rect$1();

var _contentSizeExtend = jsonAsset.Size$1.ZERO.clone();

var _nodeContentSize = jsonAsset.Size$1.ZERO.clone();

var _startPosition = jsonAsset.Vec2.ZERO.clone();

var _drawUnderlinePos = jsonAsset.Vec2.ZERO.clone();
...
var ttfUtils = {
  getAssemblerData: function getAssemblerData() {
    var sharedLabelData = sprite.Label._canvasPool.get();

    return sharedLabelData;
  },
  resetAssemblerData: function resetAssemblerData(assemblerData) {
    if (assemblerData) {
      sprite.Label._canvasPool.put(assemblerData);
    }
  },
  updateRenderData: function updateRenderData(comp) {
    if (!comp.renderData || !comp.renderData.vertDirty) {
  ...
  _updateProperties: function _updateProperties(comp, trans) {
    ...
    _outlineComp = deprecated.LabelOutline && comp.getComponent(deprecated.LabelOutline);
    ...
    _shadowComp = deprecated.LabelShadow && comp.getComponent(deprecated.LabelShadow);
  ...
  _calculateFillTextStartPosition: function _calculateFillTextStartPosition() {
    ...
    if (_hAlign$1 === sprite.HorizontalTextAlignment.RIGHT) {
      labelX = _canvasSize.width - _canvasPadding.width;
    } else if (_hAlign$1 === sprite.HorizontalTextAlignment.CENTER) {
    ...
    var firstLinelabelY = _fontSize$1 * (1 - sprite.BASELINE_RATIO / 2);

    if (_vAlign$1 !== sprite.VerticalTextAlignment.TOP) {
      var blank = drawStartY + _canvasPadding.height + _fontSize$1 - _canvasSize.height;

      if (_vAlign$1 === sprite.VerticalTextAlignment.BOTTOM) {
        blank += sprite.BASELINE_RATIO / 2 * _fontSize$1;
  ...
  _updateTexture: function _updateTexture(comp) {
    ...
    if (comp._srcBlendFactor === jsonAsset.BlendFactor.SRC_ALPHA) {
    ...
    if (_texture) {
      ...
      if (_texture instanceof spriteFrame.SpriteFrame) {
      ...
      if (uploadAgain) {
        ...
        if (_texture instanceof spriteFrame.SpriteFrame) {
          _texture.rect = new jsonAsset.Rect$1(0, 0, _canvas.width, _canvas.height);

          _texture._calculateUV();
        }

        if (jsonAsset.legacyCC.director.root && jsonAsset.legacyCC.director.root.batcher2D) {
          jsonAsset.legacyCC.director.root.batcher2D._releaseDescriptorSetCache(tex.getHash());
        }
      }
    }
  },
  _resetDynamicAtlas: function _resetDynamicAtlas(comp) {
    if (comp.cacheMode !== sprite.Label.CacheMode.BITMAP) return;
    var frame = comp.ttfSpriteFrame;
    spriteFrame.dynamicAtlasManager.deleteAtlasSpriteFrame(frame);
  ...
  _calDynamicAtlas: function _calDynamicAtlas(comp) {
    if (comp.cacheMode !== sprite.Label.CacheMode.BITMAP) return;
    var frame = comp.ttfSpriteFrame;
    spriteFrame.dynamicAtlasManager.packToDynamicAtlas(comp, frame);
  ...
  _calculateParagraphLength: function _calculateParagraphLength(paragraphedStrings, ctx) {
    ...
    for (var _iterator = jsonAsset._createForOfIteratorHelperLoose(paragraphedStrings), _step; !(_step = _iterator()).done;) {
      var para = _step.value;
      var width = sprite.safeMeasureText(ctx, para, _fontDesc);
  ...
  _measureText: function _measureText(ctx, fontDesc) {
    return function (string) {
      return sprite.safeMeasureText(ctx, string, fontDesc);
    };
  },
    ...
    if (_isWrapText$1) {
      ...
      while (left < right) {
        ...
        for (i = 0; i < paragraphedStrings.length; ++i) {
          var allWidth = sprite.safeMeasureText(_context, paragraphedStrings[i], _fontDesc);
          textFragment = sprite.fragmentText(paragraphedStrings[i], allWidth, canvasWidthNoMargin, this._measureText(_context, _fontDesc));
  ...
  _calculateWrapText: function _calculateWrapText(paragraphedStrings) {
    ...
    for (var i = 0; i < paragraphedStrings.length; ++i) {
      var allWidth = sprite.safeMeasureText(_context, paragraphedStrings[i], _fontDesc);
      var textFragment = sprite.fragmentText(paragraphedStrings[i], allWidth, canvasWidthNoMargin, this._measureText(_context, _fontDesc));
      ...
      case Overflow.NONE:
        {
          ...
          for (var i = 0; i < paragraphedStrings.length; ++i) {
            var paraLength = sprite.safeMeasureText(_context, paragraphedStrings[i], _fontDesc);
            canvasSizeX = canvasSizeX > paraLength ? canvasSizeX : paraLength;
          }

          canvasSizeY = (_splitStrings.length + sprite.BASELINE_RATIO) * this._getLineHeight();
      ...
      case Overflow.RESIZE_HEIGHT:
        {
          ...
          var _rawHeight = (_splitStrings.length + sprite.BASELINE_RATIO) * this._getLineHeight();
...
var WHITE$1 = jsonAsset.Color$1.WHITE.clone();
var ttf = {
  createData: function createData(comp) {
    ...
    for (var i = 0; i < 4; i++) {
      jsonAsset.Color$1.toArray(vData, WHITE$1, offset);
      offset += 9;
    }
...
jsonAsset.addon(ttf, ttfUtils);

var labelAssembler = {
  getAssembler: function getAssembler(comp) {
    var assembler = ttf;

    if (comp.font instanceof sprite.BitmapFont) {
      assembler = bmfont;
    } else if (comp.cacheMode === sprite.Label.CacheMode.CHAR) {
      assembler = letter;
    }
...
sprite.Label.Assembler = labelAssembler;

var FillType = sprite.Sprite.FillType;
var matrix = new jsonAsset.Mat4();
var tempColor$1 = new jsonAsset.Color$1(255, 255, 255, 255);
var barFilled = {
  useModel: false,
  updateRenderData: function updateRenderData(sprite) {
    var frame = sprite.spriteFrame;
    spriteFrame.dynamicAtlasManager.packToDynamicAtlas(sprite, frame);
  ...
  createData: function createData(sprite) {
    ...
    for (var _iterator = jsonAsset._createForOfIteratorHelperLoose(dataList), _step; !(_step = _iterator()).done;) {
  ...
  updateWorldVertexData: function updateWorldVertexData(sprite) {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      jsonAsset.Vec3.transformMat4(world, local, matrix);
    }
  },
...
var PI_2 = Math.PI * 2;
var EPSILON = 1e-6;
var tempColor$2 = new jsonAsset.Color$1(255, 255, 255, 255);
var _vertPos = [new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2()];

var _vertices = new Array(4);

var _uvs = new Array(8);

var _intersectPoint_1 = [new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2()];
var _intersectPoint_2 = [new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2()];

var _center = new jsonAsset.Vec2();

var _triangles = [new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2(), new jsonAsset.Vec2()];
...
function _calculateVertices(sprite) {
  ...
  for (var _iterator = jsonAsset._createForOfIteratorHelperLoose(_triangles), _step; !(_step = _iterator()).done;) {
    var num = _step.value;
    jsonAsset.Vec2.set(num, 0, 0);
  }

  if (cx !== vertices[0]) {
    jsonAsset.Vec2.set(_triangles[0], 3, 0);
  }

  if (cx !== vertices[2]) {
    jsonAsset.Vec2.set(_triangles[2], 1, 2);
  }

  if (cy !== vertices[1]) {
    jsonAsset.Vec2.set(_triangles[1], 0, 1);
  }

  if (cy !== vertices[3]) {
    jsonAsset.Vec2.set(_triangles[3], 2, 3);
  }
}
...
for (var i$1 = 0; i$1 < 4; i$1++) {
  vec3_temps$1.push(new jsonAsset.Vec3());
}
...
var vec3_temp$1 = new jsonAsset.Vec3();
var matrix$1 = new jsonAsset.Mat4();
var sliced = {
  useModel: false,
  createData: function createData(sprite) {
    var renderData = sprite.requestRenderData();
  ...
  fillBuffers: function fillBuffers(sprite, renderer) {
    ...
    for (var i = 4; i < 20; ++i) {
      ...
      jsonAsset.Color$1.toArray(vBuf, dataList[i].color, vertexOffset);
      vertexOffset += 4;
    }
  ...
  updateWorldVertexData: function updateWorldVertexData(sprite) {
    ...
    for (var row = 0; row < 4; ++row) {
      ...
      for (var col = 0; col < 4; ++col) {
        ...
        jsonAsset.Vec3.set(vec3_temp$1, colD.x, rowD.y, 0);
        jsonAsset.Vec3.transformMat4(world, vec3_temp$1, matrix$1);
      }
    }
  },
  updateColor: function updateColor(sprite) {
...
for (var i$2 = 0; i$2 < 4; i$2++) {
  vec3_temps$2.push(new jsonAsset.Vec3());
}
  ...
  fillBuffers: function fillBuffers(sprite, renderer) {
    ...
    for (var yIndex = 0, yLength = row; yIndex < yLength; ++yIndex) {
      ...
      for (var xIndex = 0, xLength = col; xIndex < xLength; ++xIndex) {
        ...
        jsonAsset.Color$1.toArray(vBuf, datalist[0].color, vertexOffsetV + 1);
        jsonAsset.Color$1.toArray(vBuf, datalist[0].color, vertexOffsetV + offset1 + 1);
        jsonAsset.Color$1.toArray(vBuf, datalist[0].color, vertexOffsetV + offset2 + 1);
        jsonAsset.Color$1.toArray(vBuf, datalist[0].color, vertexOffsetV + offset3 + 1);
  ...
  fillVertices: function fillVertices(vBuf, vertexOffset, matrix, row, col, dataList) {
    ...
    for (var yIndex = 0, yLength = row; yIndex < yLength; ++yIndex) {
      ...
      for (var xIndex = 0, xLength = col; xIndex < xLength; ++xIndex) {
        ...
        jsonAsset.Vec3.set(vec3_temps$2[0], x, y, 0);
        jsonAsset.Vec3.set(vec3_temps$2[1], x1, y, 0);
        jsonAsset.Vec3.set(vec3_temps$2[2], x, y1, 0);
        jsonAsset.Vec3.set(vec3_temps$2[3], x1, y1, 0);

        for (var _i2 = 0; _i2 < 4; _i2++) {
          var vec3_temp = vec3_temps$2[_i2];
          jsonAsset.Vec3.transformMat4(vec3_temp, vec3_temp, matrix);
...
var SpriteType = sprite.Sprite.Type;
var FillType$1 = sprite.Sprite.FillType;
var spriteAssembler = {
...
sprite.Sprite.Assembler = spriteAssembler;

var _stencilManager = renderable2d.StencilManager.sharedManager;
...
function applyAreaMask(mask, renderer) {
  ...
  if (mask.type === deprecated.MaskType.IMAGE_STENCIL) {
...
var maskAssembler = {
  ...
  fillBuffers: function fillBuffers(mask, renderer) {
    if (mask.type !== deprecated.MaskType.IMAGE_STENCIL || mask.spriteFrame) {
...
deprecated.Mask.Assembler = StartAssembler;
deprecated.Mask.PostAssembler = PostAssembler;
...
var Batcher2D = function () {
  ...
  _proto.acquireBufferBatch = function acquireBufferBatch(attributes) {
    ...
    var strideBytes = attributes === vertexFormat.vfmtPosUvColor ? 36 : vertexFormat.getAttributeStride(attributes);
  ...
  _proto.registerCustomBuffer = function registerCustomBuffer(attributes, callback) {
    ...
    if (attributes instanceof deprecated.MeshBuffer) {
      batch = attributes;
    } else {
  ...
  jsonAsset._createClass(Batcher2D, [{
    key: "currBufferBatch",
    get: function get() {
  ...
  function Batcher2D(_root) {
    ...
    this.device = void 0;
    this._screens = [];
    this._bufferBatchPool = new jsonAsset.RecyclePool(function () {
      return new deprecated.MeshBuffer(_this);
    }, 128);
    ...
    this._emptyMaterial = new jsonAsset.Material();
    ...
    this._batches = new jsonAsset.CachedArray(64);
    this._drawBatchPool = new jsonAsset.Pool(function () {
      return new deprecated.DrawBatch2D();
    }, 128);
  }
  ...
  _proto.destroy = function destroy() {
    ...
    for (var _iterator = jsonAsset._createForOfIteratorHelperLoose(this._meshBuffers.keys()), _step; !(_step = _iterator()).done;) {
    ...
    renderable2d.StencilManager.sharedManager.destroy();
  };
  ...
  _proto.reset = function reset() {
    ...
    renderable2d.StencilManager.sharedManager.reset();
  ...
  _proto.commitComp = function commitComp(comp, frame, assembler, transform) {
    ...
    renderComp.stencilStage = renderable2d.StencilManager.sharedManager.stage;
  ...
  _proto.commitModel = function commitModel(comp, model, mat) {
    ...
    if (mat) {
      if (comp.stencilStage === renderable2d.Stage.ENABLED || comp.stencilStage === renderable2d.Stage.DISABLED) {
        comp.stencilStage = renderable2d.StencilManager.sharedManager.stage;
      }

      depthStencil = renderable2d.StencilManager.sharedManager.getStencilStage(comp.stencilStage, mat);
      dssHash = renderable2d.StencilManager.sharedManager.getStencilHash(comp.stencilStage);
    }
  ...
  _proto.autoMergeBatches = function autoMergeBatches(renderComp) {
    ...
    if (renderComp) {
      ...
      if (renderComp.customMaterial !== null) {
        depthStencil = renderable2d.StencilManager.sharedManager.getStencilStage(renderComp.stencilStage, mat);
      } else {
        depthStencil = renderable2d.StencilManager.sharedManager.getStencilStage(renderComp.stencilStage);
      }

      dssHash = renderable2d.StencilManager.sharedManager.getStencilHash(renderComp.stencilStage);
    }
    ...
    if (jsonAsset.sys.__isWebIOS14OrIPadOS14Env && !this._currIsStatic) {
      this._currMeshBuffer = null;
    }
  };
...
var LocalDescriptorSet = function () {
  jsonAsset._createClass(LocalDescriptorSet, [{
    key: "descriptorSet",
    get: function get() {
  ...
  function LocalDescriptorSet() {
    ...
    var device = jsonAsset.legacyCC.director.root.device;
    this._localData = new Float32Array(jsonAsset.UBOLocal.COUNT);
    this._localBuffer = device.createBuffer(new jsonAsset.BufferInfo(jsonAsset.BufferUsageBit.UNIFORM | jsonAsset.BufferUsageBit.TRANSFER_DST, jsonAsset.MemoryUsageBit.HOST | jsonAsset.MemoryUsageBit.DEVICE, jsonAsset.UBOLocal.SIZE, jsonAsset.UBOLocal.SIZE));
  }
  ...
  _proto2.initialize = function initialize(batch) {
    ...
    this._descriptorSet.bindBuffer(jsonAsset.UBOLocal.BINDING, this._localBuffer);

    var binding = jsonAsset.ModelLocalBindings.SAMPLER_SPRITE;
...
var DescriptorSetCache = function () {
  function DescriptorSetCache() {
    ...
    this._localCachePool = new jsonAsset.Pool(function () {
  ...
  _proto3.destroy = function destroy() {
    this._descriptorSetCache.forEach(function (value, key, map) {
      value.forEach(function (descriptorSet) {
...
jsonAsset.legacyCC.internal.Batcher2D = Batcher2D;
...
function checkFontLoaded() {
  ...
  for (var i = _loadingFonts.length - 1; i >= 0; i--) {
    ...
    if (now - fontLoadHandle.startTime > _timeout) {
      jsonAsset.warnID(4933, fontFamily);
    ...
    var newWidth = sprite.safeMeasureText(_canvasContext, _testString, fontDesc);
...
function loadFont(url, options, onComplete) {
  ...
  var refWidth = sprite.safeMeasureText(_canvasContext, _testString, fontDesc);
...
jsonAsset.legacyCC.UI = {
  MeshBuffer: deprecated.MeshBuffer,
...
exports.graphicsAssemblerManager = graphicsAssemblerManager;
